...
ds = vcl.load_data.ds.where((vcl.load_data.ds.conc >= 0) | vcl.load_data.ds.isnull(), other=0)
bodem0 = np.array(vcl.load_data.ds_b0[0, 1435:2466, 1201:2970])
bodem0[np.where(bodem0 == -9999)] = -43.8

# Scale satellite image to bathymetry shapes
sat_scaled = cv2.resize(
    vcl.load_data.sat, dsize=(bodem0.shape[1], bodem0.shape[0]), interpolation=cv2.INTER_CUBIC
)
